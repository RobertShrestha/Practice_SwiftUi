...
struct C3CheckoutView: View {
    ...
    func placeOrder() {
        ...
        URLSession.shared.dataTask(with:request) { data , response , error in
            ...
            if let orderDecoded = try? JSONDecoder().decode(OrderStruct.self, from: data) {
...
struct C3CheckoutView_Previews: PreviewProvider {
    static var previews: some View {
        C3CheckoutView(order: MyOrder())
    }
}
